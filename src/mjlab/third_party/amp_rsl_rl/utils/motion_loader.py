...
class AMPLoader:
  ...
  def get_frame_at_time_batch(self, traj_idxs, times):
    ...
    all_frame_ends = torch.zeros_like(all_frame_starts)

    for traj_idx in np.unique(traj_idxs):
      mask = traj_idxs == traj_idx
      trajectory = self.trajectories[traj_idx]
      all_frame_starts[mask] = trajectory[idx_low[mask]]
      all_frame_ends[mask] = trajectory[idx_high[mask]]

    blend = torch.tensor(idx - idx_low, device=self.device).clamp(0, 1).unsqueeze(-1)
  ...
  def get_full_frame_at_time_batch(self, traj_idxs, times):
    ...
    n = self.trajectory_num_frames[traj_idxs]
    idx_low, idx_high = (
      np.floor(p * n).astype(np.int64),
      np.ceil(p * n).astype(np.int64),
    )
  ...
  def blend_frame_pose(self, frame0, frame1, blend):
    ...
    joints0, joints1 = (
      AMPLoader.get_joint_pose(frame0),
      AMPLoader.get_joint_pose(frame1),
    )
    joint_vel_0, joint_vel_1 = (
      AMPLoader.get_joint_vel(frame0),
    ...
    for _ in range(num_mini_batch):
      if self.preload_transitions:
        ...
        for traj_idx, frame_time in zip(traj_idxs, times):
          ...
          s_next.append(
            self.get_frame_at_time(traj_idx, frame_time + self.time_between_frames)
          )
