...
class AMPLoaderDisplay:
    ...
    for i, motion_file in enumerate(motion_files):
      ...
      with open(motion_file) as f:
        ...
        self.trajectories.append(
          torch.tensor(
            motion_data[:, : AMPLoaderDisplay.JOINT_VEL_END_IDX],
            dtype=torch.float32,
            device=device,
          )
        )
  ...
  def blend_frame_pose(self, frame0, frame1, blend):
    ...
    """
    joints0, joints1 = (
      AMPLoaderDisplay.get_joint_pose(frame0),
      AMPLoaderDisplay.get_joint_pose(frame1),
    )
    joint_vel_0, joint_vel_1 = (
      AMPLoaderDisplay.get_joint_vel(frame0),
      AMPLoaderDisplay.get_joint_vel(frame1),
    )
  ...
  def get_joint_pose(pose):
    return pose[
      AMPLoaderDisplay.JOINT_POSE_START_IDX : AMPLoaderDisplay.JOINT_POSE_END_IDX
    ]
  ...
  def get_joint_vel(pose):
    return pose[
      AMPLoaderDisplay.JOINT_VEL_START_IDX : AMPLoaderDisplay.JOINT_VEL_END_IDX
    ]
