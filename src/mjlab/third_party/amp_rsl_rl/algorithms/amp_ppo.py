...
class AMPPPO:
  ...
  def update(self):  # noqa: C901
    ...
    for sample, sample_amp_policy, sample_amp_expert in zip(
      ...
      if self.amp_normalizer is not None:
        # Update normalizer with raw (pre-normalization) samples.
  ...
  def broadcast_parameters(self):
    ...
    broadcast_module_parameters(self.discriminator, src=0)
    if self.rnd:
  ...
  def reduce_parameters(self):
    ...
    all_params = list(self.policy.parameters()) + list(self.discriminator.parameters())
    if self.rnd:
