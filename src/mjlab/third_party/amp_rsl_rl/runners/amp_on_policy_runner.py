...
from mjlab.third_party.amp_rsl_rl.algorithms import AMPPPO
...
class AmpOnPolicyRunner:
  ...
  def __init__(
    ...
    min_std = torch.zeros(
      len(train_cfg["min_normalized_std"]), device=self.device, requires_grad=False
    )
  ...
  def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):  # noqa: C901
    ...
    for it in range(start_iter, tot_iter):
      ...
      # synchronize running normalizers across ranks (for consistent inputs)
      if self.is_distributed and self.empirical_normalization:
        distributed_sync_empirical_normalization(self.obs_normalizer)
        distributed_sync_empirical_normalization(self.privileged_obs_normalizer)
      if self.is_distributed and getattr(self.alg, "amp_normalizer", None) is not None:
        distributed_sync_running_mean_std(self.alg.amp_normalizer, device=self.device)
  ...
  def log(self, locs: dict, width: int = 80, pad: int = 35):
    ...
    # helpers for distributed mean reductions (sum/count)
    def _dist_mean(val: torch.Tensor, n: int) -> torch.Tensor:
      if not self.is_distributed:
        return val
      v = val.detach().to(self.device, dtype=torch.float64) * float(n)
      c = torch.tensor(float(n), device=self.device, dtype=torch.float64)
      ...
      return (v / torch.clamp(c, min=1.0)).to(dtype=val.dtype)

    # -- Episode info (aggregate across ranks)
    ep_string = ""
    if locs["ep_infos"]:
      for key in locs["ep_infos"][0]:
        ...
        value = (
          torch.mean(infotensor)
          if local_n > 0
          else torch.tensor(0.0, device=self.device)
        )
        value = _dist_mean(value, local_n)
    ...
    # -- Training (aggregate rolling buffers across ranks)
    local_rew_n = len(locs["rewbuffer"])
    local_len_n = len(locs["lenbuffer"])
    if local_rew_n > 0:
    ...
    global_rew_mean = _dist_mean(local_rew_mean, local_rew_n)
    global_len_mean = _dist_mean(local_len_mean, local_len_n)
    ...
    if not self.disable_logs and (local_rew_n > 0 or self.is_distributed):
      if self.alg.rnd:
        ...
        erew_mean = _dist_mean(erew_mean, len(locs["erewbuffer"]))
        irew_mean = _dist_mean(irew_mean, len(locs["irewbuffer"]))
      ...
      self.writer.add_scalar("Train/mean_reward", global_rew_mean, locs["it"])
      self.writer.add_scalar("Train/mean_episode_length", global_len_mean, locs["it"])
      if self.logger_type != "wandb":
        self.writer.add_scalar("Train/mean_reward/time", global_rew_mean, self.tot_time)
        self.writer.add_scalar(
          "Train/mean_episode_length/time", global_len_mean, self.tot_time
        )
    ...
    if local_rew_n > 0:
      ...
      log_string += f"""{"Mean reward:":>{pad}} {global_rew_mean.item():.2f}\n"""
      # -- episode info
      log_string += (
        f"""{"Mean episode length:":>{pad}} {global_len_mean.item():.2f}\n"""
      )
    else:
      log_string = (
        f"""{"#" * width}\n"""
