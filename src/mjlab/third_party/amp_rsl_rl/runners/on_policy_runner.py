...
from mjlab.third_party.amp_rsl_rl.algorithms import PPO, Distillation
  ...
  def log(self, locs: dict, width: int = 80, pad: int = 35):
    ...
    if locs["ep_infos"]:
      for key in locs["ep_infos"][0]:
        ...
        # log to logger and terminal (rank0 only)
    ...
    # -- Training
    # -- Training (aggregate rolling buffers across ranks by sum/count)
    ...
    if local_rew_n > 0 or (self.is_distributed and not self.disable_logs):
      ...
      if not self.disable_logs:
        ...
        if (
          ...
          self.writer.add_scalar(
            "Train/mean_reward/time", global_rew_mean, self.tot_time
          )
          self.writer.add_scalar(
