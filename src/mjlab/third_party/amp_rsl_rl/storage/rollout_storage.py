...
from mjlab.third_party.amp_rsl_rl.utils import split_and_pad_trajectories
...
class RolloutStorage:
  ...
  def __init__(
    ...
    device="cpu",
    next_privileged_obs_shape=None,
  ):
    # store inputs
    ...
    if privileged_obs_shape is not None:
      ...
      _next_shape = (
        next_privileged_obs_shape
        if next_privileged_obs_shape is not None
        else privileged_obs_shape
      )
      self.next_privileged_observations = torch.zeros(
        num_transitions_per_env, num_envs, *_next_shape, device=self.device
      )
    else:
      self.privileged_observations = None
  ...
  def add_transitions(self, transition: Transition):
    ...
    if self.privileged_observations is not None:
      ...
      if transition.next_critic_observations is not None:
    ...
    for epoch in range(num_epochs):
      for i in range(num_mini_batches):
        ...
        yield (
          ...
          next_privileged_observations_batch,
          actions_batch,
          target_values_batch,
          advantages_batch,
          returns_batch,
